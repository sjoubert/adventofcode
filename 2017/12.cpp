...
static void DiscoverNeighbors(std::map<int, std::vector<int>> const& p_connections,
  std::set<int>& p_groupZero, int p_node)
{
  for (auto neighbor: p_connections.at(p_node))
  {
    if (p_groupZero.find(neighbor) == p_groupZero.end())
    {
      p_groupZero.insert(neighbor);
      DiscoverNeighbors(p_connections, p_groupZero, neighbor);
    }
  }
}
...
int main()
{
  std::map<int, std::vector<int>> const CONNECTIONS = {
    {0, {950, 1039}},
    {1, {317, 904, 923}},
    {2, {2}},
    {3, {870}},
  ...
  for (auto [id, connections]: CONNECTIONS)
  {
    std::set<int> group;
    group.insert(id);
    DiscoverNeighbors(CONNECTIONS, group, id);
    groups.insert(group);
    if (id == 0)
    {
      std::cout << group.size() << '\n';
