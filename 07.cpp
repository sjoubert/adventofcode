...
void ComputeTowerWeight(std::map<std::string, int>& p_towerWeight,
  std::map<std::string, std::pair<int, std::vector<std::string>>> const& p_data, std::string const& p_base)
{
  if (p_towerWeight.find(p_base) == p_towerWeight.end())
  {
    p_towerWeight[p_base] = p_data.at(p_base).first;
    if (p_data.at(p_base).second.empty() == false)
    {
      ...
      auto delta = weights.back() - weights.front();
      auto faultyName = weightNames[weights.front()];
      if (weights.front() == weights[weights.size() / 2])
      {
        faultyName = weightNames[weights.back()];
        delta *= -1;
      }
      if (delta != 0)
      {
        throw p_data.at(faultyName).first + delta;
      }
    }
  }
}
...
int main()
{
  std::map<std::string, std::pair<int, std::vector<std::string>>> const DATA = {
    {"bxlur", {38, {}}},
    {"vgeifn", {90, {}}},
  ...
  std::vector<std::string> flying;
  for (auto const& [holdingName, holdingData]: DATA)
  {
    for (auto const& flyingName: holdingData.second)
    {
      flying.push_back(flyingName);
    }
  }
  std::string base;
  for (auto const& [holdingName, holdingData]: DATA)
  {
    auto found = std::find(flying.begin(), flying.end(), holdingName);
    if (found == flying.end())
    {
      base = holdingName;
    }
  ...
  std::map<std::string, int> towerWeight;
  try
  {
        ComputeTowerWeight(towerWeight, DATA, base);
  }
  catch (int const& fixedWeight)
  {
    std::cout << fixedWeight;
  }
